...
import { supabase, type Team, type Wig, type MetricDef, type Practice, type IndicatorType, type User, type ChangeLogEntry } from '@/lib/supabase';
...
export default function ManagePage() {
  ...
  const [recentLogs, setRecentLogs] = useState<ChangeLogEntry[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [wigRes, metricsRes, practicesRes, usersRes, logsRes] = await Promise.all([
        ...
        supabase.from('change_log').select('*').eq('team_id', teamData.id).order('created_at', { ascending: false }).limit(20),
      ]);
  ...
  const teamUsers = users.filter(u => u.team_id === team.id || u.role === 'team_leader' || u.role === 'plant_manager');
  ...
  return (
    <main className="max-w-3xl mx-auto p-3 md:p-6 pb-20">
      <header className="mb-4">
        ...
        <div className="flex items-baseline justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-3">
            <Link href={`/team/${teamCode}/dashboard`} className="text-xs px-3 py-1.5 bg-blue-600 text-white rounded hover:bg-blue-700">📊 대시보드</Link>
      ...
      <section className="mb-6">
        <h2 className="text-sm font-medium text-gray-700 mb-2">팀 가중목 (WIG)</h2>
        {wig ? (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 md:p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" step="0.01" value={wig.baseline_value ?? ''}
              ...
              <div>
                ...
                <input type="number" step="0.01" value={wig.target_value}
              ...
              <div>
                ...
                <input type="text" value={wig.unit}
              ...
              <div>
                ...
                <select value={wig.direction} onChange={(e) => updateWig('direction', e.target.value)}
                  className="w-full text-sm border border-blue-200 rounded px-2 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-300">
              ...
              <div>
                ...
                <input type="date" value={wig.deadline ?? ''}
              ...
              <div>
                ...
                <input type="text" placeholder="예: 5월말" value={wig.deadline_label ?? ''}
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-sm text-gray-500">가중목이 아직 설정되지 않았습니다.</div>
        )}
      </section>
      ...
      <section className="mb-6">
        ...
        <MetricList metrics={leadMetrics} users={teamUsers} onUpdate={updateMetric} onDelete={deleteMetric} colorClass="bg-orange-50 border-orange-200" />
      </section>
      ...
      <section className="mb-6">
        ...
        <MetricList metrics={lagMetrics} users={teamUsers} onUpdate={updateMetric} onDelete={deleteMetric} colorClass="bg-blue-50 border-blue-200" />
      </section>
      ...
      {generalMetrics.length > 0 && (
        <section className="mb-6">
          ...
          <MetricList metrics={generalMetrics} users={teamUsers} onUpdate={updateMetric} onDelete={deleteMetric} colorClass="bg-gray-50 border-gray-200" />
        </section>
      )}
          ...
          {practices.length === 0 ? (
            ...
            <div key={p.id} className="bg-white border border-gray-200 rounded-lg p-3">
              <div className="flex items-start gap-2 mb-2">
                <input type="checkbox" checked={p.is_completed}
              ...
              <div className="flex items-center gap-2 pl-6">
                ...
                <select value={p.owner_user_id || ''}
                  onChange={(e) => updatePractice(p.id, 'owner_user_id', e.target.value || null)}
      ...
      <section className="mb-6">
        <h2 className="text-sm font-medium text-gray-700 mb-2">최근 변경 이력 (최대 20건)</h2>
        <div className="bg-white border border-gray-200 rounded-lg divide-y divide-gray-100">
          {recentLogs.length === 0 ? (
            <div className="p-4 text-center text-sm text-gray-500">아직 변경 이력이 없습니다</div>
          ) : recentLogs.map(log => {
              ...
              <div key={log.id} className="p-3 text-xs">
                <div className="flex items-center justify-between mb-1 flex-wrap gap-1">
                  <div className="flex items-center gap-2">
                    <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium ${actionColor}`}>{summary.actionLabel}</span>
                    <span className="font-medium text-gray-800">{summary.entityLabel}</span>
                  </div>
                  <span className="text-gray-400 text-[10px]">{formatRelativeTime(log.created_at)}</span>
                </div>
                <ul className="text-gray-600 space-y-0.5 pl-2">
                  {summary.details.map((d, i) => <li key={i} className="text-[11px]">• {d}</li>)}
                </ul>
              </div>
            );
          })}
        ...
        {recentLogs.length >= 20 && (
          <div className="text-xs text-gray-500 mt-2 text-center">
            <Link href="/admin/history" className="text-blue-600 hover:underline">전체 이력 보기 →</Link>
          </div>
        )}
      </section>

      <div className="bg-amber-50 border-l-4 border-amber-400 p-3 text-xs text-amber-900">
        <div className="font-medium mb-1">자동 저장 + 변경이력 자동 기록</div>
        <div>입력 칸을 벗어나는 순간 자동으로 저장되고, 모든 변경은 위의 "최근 변경 이력"에 자동 기록됩니다.</div>
      </div>
    </main>
  );
}
        ...
        <div key={m.id} className={`${colorClass} border rounded-lg p-3`}>
          <div className="grid grid-cols-12 gap-2 items-end mb-2">
            ...
            <div className="col-span-4 md:col-span-2">
              ...
              <input type="number" step="0.01" value={m.target_value}
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-gray-100">
            ...
            <select value={m.owner_user_id || ''}
              onChange={(e) => onUpdate(m.id, 'owner_user_id', e.target.value || null)}
              className="text-xs border border-gray-200 rounded px-2 py-0.5 bg-white flex-1 max-w-xs">
              <option value="">미지정</option>
